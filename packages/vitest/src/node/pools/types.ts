...
export interface ProcessPool {
  name: string
  runTests: (files: TestSpecification[], invalidates?: string[]) => Awaitable<void>
  collectTests: (files: TestSpecification[], invalidates?: string[]) => Awaitable<void>
...
export type WorkerRequest
  = { __vitest_worker_request__: true } & (
    | { type: 'start'; options: { workerFilename: string; workerId: number; reportMemory: boolean } }
    | { type: 'stop' }
    | { type: 'run'; context: ContextRPC }
    | { type: 'collect'; context: ContextRPC }
    | { type: 'cancel' }
)
